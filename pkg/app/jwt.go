...
type Claims struct {
	// 所嵌入的 AppKey 和 AppSecret，
	AppKey    string `json:"app_key"`
	AppSecret string `json:"app_secret"`
	// jwt-go 库中预定义的，也是 JWT 的规范
	jwt.StandardClaims
}
...
/*
	在 GenerateToken 方法中，它承担了整个流程中比较重要的职责，也就是生成 JWT Token 的行为，
    主体的函数流程逻辑是根据客户端传入的 AppKey 和 AppSecret 以及在项目配置中所设置的签发者（Issuer）和过期时间（ExpiresAt），
    根据指定的算法生成签名后的 Token。这其中涉及两个的内部方法，如下：
		jwt.NewWithClaims：根据 Claims 结构体创建 Token 实例，它一共包含两个形参，
			第一个参数是 SigningMethod，其包含 SigningMethodHS256、SigningMethodHS384、SigningMethodHS512 三种 crypto.Hash 加密算法的方案。
...
// ParseToken 它主要的功能是解析和校验 Token，承担着与 GenerateToken 相对的功能，其函数流程主要是解析传入的 Token，然后根据 Claims 的相关属性要求进行校验
/*
	这其中涉及两个的内部方法，如下：
